...
DRY_RUN=0
REUSE_WINDOW_QK_BRIDGE="${GOLEM_ATTENTION_REUSE_WINDOW_QK_BRIDGE:-0}"
WORKER_CLUSTER_BRIDGE="${GOLEM_ATTENTION_WORKER_CLUSTER_BRIDGE:-0}"
WORKER_CLUSTER_QK_WORKERS_PER_MANAGER="${GOLEM_ATTENTION_WORKER_CLUSTER_QK_WORKERS_PER_MANAGER:-1}"
if [[ "$WORKER_CLUSTER_QK_WORKERS_PER_MANAGER" != "1" &&
      "$WORKER_CLUSTER_QK_WORKERS_PER_MANAGER" != "2" ]]; then
...
RUN_CMD=(timeout "$TIMEOUT_SECONDS" env
  ...
  "GOLEM_ARCH_SCRIPT=$([[ $REUSE_WINDOW_QK_BRIDGE == 1 ]] && echo architecture/ncores_selfcom_dma_ctrl.py || echo architecture/archive/ncores_selfcom_dma.py)"
  ...
  "GOLEM_ATTENTION_REUSE_WINDOW_QK_BRIDGE=$REUSE_WINDOW_QK_BRIDGE"
  ...
  "GOLEM_CTRL_LINK_ENABLE=$((KV_DISTRIBUTION || REUSE_WINDOW_QK_BRIDGE))"
  "GOLEM_REQUEST_SCHEDULER_ENABLE=$REUSE_WINDOW_QK_BRIDGE"
  "GOLEM_A_REUSE_N_TILES=$((REUSE_WINDOW_QK_BRIDGE ? 4 : 1))"
  "GOLEM_B_REUSE_M_TILES=$((REUSE_WINDOW_QK_BRIDGE ? 2 : 1))"
  "GOLEM_DMA_SLOT_COUNT=$((REUSE_WINDOW_QK_BRIDGE ? 24 : 4))"
...
if (( REUSE_WINDOW_QK_BRIDGE )); then
  ...
  attention_ui_key_value "Artifacts" "$ARTIFACT_ROOT"
  attention_ui_key_value "Reuse-window result" "$QK_BRIDGE_RESULT"
  exit 0
fi
